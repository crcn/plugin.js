...
if(globalExists) global.pluginPaths = global.pluginPaths || [];
...
module.exports = function(plugInto) {
	...
	//second param is for debugging - soft-linked plugin path breaks node module searching
	var paths       = [__dirname], 
	...
	var resolve = function(jsPath) {
		...
		return resolveFromPaths(jsPath, paths) || (globalExists ? resolveFromPaths(jsPath, global.pluginPaths) : null);
	}
	...
	function onPluginLoaded(err, ops) {
		...
		var instance = ops.plugin || (self.newPlugin(module, plugInto, self) || {}),
	...
	_.extend(self, {
		...
		'params': function(name, value) {
			...
			if(arguments.length == 1 && typeof arguments[0] == 'string') return dref.get(params, name);
		...
		'newPlugin': function(module, options, loader) {
			return module.plugin.call(module, options, loader);
		},
...
module.exports.paths = function() {
	if(arguments.length) {
		includePaths(Array.apply(null, arguments), global.pluginPaths);
	} else {
		return paths;
	}
}