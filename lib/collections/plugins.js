...
module.exports = structr({
	...
	"loadModules": function(q, callback) {
		async.map(sift(this._query(q), this._pluginLoaders), function(loader, next) {
			loader.load(next);
		}, callback);
	},
	...
	"loadDeps": function(deps, next) {
		...
		async.map(deps || [], function(dep, next) {
		...
		return (deps || []).map(function(dep) {
			...
			dep.names.map(function(name) {
				modules = modules.concat(self.modules(name));
			});
			if(dep.multi) {
				return modules;
			} else {
	...
	"load": function(callback) {
		async.forEach(this._pluginLoaders, function(loader, next) {
			loader.load(next);
		}, callback);
