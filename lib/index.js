			...
			Array.apply(null, arguments).forEach(function(plugin) {
				for(var i = self.loaders.length; i--;) {
					...
					if(loader.test(plugin)) {
						return loader.load(plugin, function(instance, name) {
							var instance = instance.plugin(options, params[name] || {});
							plugins[name] = {
								instance: instance,
								require: instance.require = normalizeRequired(instance.require),
								path: plugin
							};
						});
					}
			...
			if(!initialized) {
				...
				for(var name in plugins) {
					...
					(plugin.require || []).forEach(function(req) {
						...
						if(!plugins.length) throw new Error('Unable to find plugin: ' + req.name + ' in ' + name);

						//include the instance if it exists
						if(req.name) plugin.require[req.name] = {
							instance: plugins[0],
							instances: plugins
						};
					});
